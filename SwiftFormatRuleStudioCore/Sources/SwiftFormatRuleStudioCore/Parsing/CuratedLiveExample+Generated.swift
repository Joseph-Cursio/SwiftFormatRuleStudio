...
extension CuratedLiveExample {
    ...
    static let generatedHints: [String: String] = [
        ...
        "indent": "Toggle the options above to watch the indentation change: indent-case shifts the switch cases, indent-strings indents the multiline string body, and ifdef controls the #if block.",
        "organizeDeclarations": "The class, struct and enum each get reordered and MARK-ed by default. Raise a `*-threshold` above a body's line count to stop organizing that kind, or change `--visibility-order` / `--organization-mode` to re-sort the groups.",
        ...
        "wrapArguments": "This rule realigns *existing* wrapping at any width — the misaligned parameters below are fixed even with no max-width. Set `--max-width` (e.g. 40) to also wrap the long collection, parameter list, function effects (async/throws) and return type, then toggle the matching wrap option to see each style change.",
