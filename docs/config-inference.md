...
| **New-rule free-win digest on upgrade** — newly-added opt-in rules that are zero-churn for you | Upgrade inflection, recurring | `TuneModel` + a static per-version catalog — needs no second engine ([scope](version-upgrade-diff-scope.md) §2) | **shipped** — Tune tab, "New since \<version>" |
